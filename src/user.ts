...
const debug = createDebug('report-backend:user');
...
export default class User {
    ...
    public state: string;
    /**
     * ReportAPI認証クライアント(管理者としてのAuthorizationCodeフロー)
     */
    public authClient: api.auth.OAuth2;
    public profile: IProfile;
    constructor(configurations: IConfigurations) {
        this.host = configurations.host;
        this.session = configurations.session;
        this.authClient = new api.auth.OAuth2({
